...
import re
from regextractor.utils import *
    ...
    if type(tree) == dict:
        if tree["type"] == "chars":
            ...
            if count > 1:
                out += "{%d}" % count
            return (
                tree_to_regex(tree["left"]) + out + tree_to_regex(tree["right"])
            )
    else:
...
def tree_to_HTML(tree):
    ...
    if type(tree) == dict:
        if tree["type"] == "chars":
            return (
                tree_to_HTML(tree["left"])
                + '<span class="rgt-exact-match rgt-tooltip">'
                + tree["root"].replace(
                    " ",
                    '<span class="rgt-space-char rgt-tooltip" title="Space character"> </span>',
                )
                + "</span>"
            ...
            if count > 1:
                ...
                out += "</span>"
            return (
                tree_to_HTML(tree["left"]) + out + tree_to_HTML(tree["right"])
            )
    else:
        cs, m, M = strs_range(tree)
        if not M:
...
if __name__ == "__main__":
    ...
    print(
        tree_to_regex(gen_tree([s1, s2, s3]))
    )  # [a-z]b[a-z][!@#$%^&*()_+=-`~'";:,<.>/?\\]}\[{][0-9]{0,3}0[0-9]{0,4}
